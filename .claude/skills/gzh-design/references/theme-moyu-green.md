...
**签名文案适配**：SKILL.md 的作者签名（"我是艾启蒙，用AI陪伴成长，用技术创造价值！"两段，默认写死、仅用户明确给出时替换）以正文段落（组件 5）形式放在 footer-cta **之前**；footer-cta 内部文案保持上面的固定句式，两者不重复出现"三连"字样时可将签名第二段并入 footer-cta 顶部文字。

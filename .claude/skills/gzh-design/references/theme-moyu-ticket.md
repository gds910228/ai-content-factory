...
- `{{作者名}}` → 默认「艾启蒙」 / `{{作者身份}}` → 默认「用AI陪伴成长，用技术创造价值！」
...
**签名文案适配**：SKILL.md 的作者签名（"我是艾启蒙，用AI陪伴成长，用技术创造价值！"两段，默认写死、仅用户明确给出时替换）以正文段落（组件 5）形式放在本组件**之前**；本组件内部 `{{互动文案}}` 直接使用 SKILL.md 固定的第二段（"如果你觉得今天这篇有收获…三连，我们下篇见"），不要重复堆叠两句"三连"。

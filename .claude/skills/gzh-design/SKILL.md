---
name: gzh-design
description: 微信公众号文章排版引擎，将 Markdown 转换为可直接粘贴到公众号编辑器的 HTML。主题风格从 references/theme-index.md 注册的自定义主题库中选取，自动章节编号、关键词下划线标记、引言卡片、目录导航、代码块、图片/GIF、作者签名。支持 Markdown / Word(.docx) / PDF / 纯文本输入（非 Markdown 先自动归一化），也支持"一键自动排版"（自动推断结构+选主题），还支持根据用户描述/参考图生成自定义主题组件库并保存本地复用。触发场景：(1) 用户提到"公众号排版""公众号文章""微信排版""gzh"，(2) 用户想把文章（md/docx/pdf/纯文本）转成公众号 HTML，(3) 用户说"自动排版""一键排版"公众号内容，(4) 用户想为公众号排版"生成新主题/自定义风格/按这张图做一套组件库"，(5) 图片为本地文件（非公网 URL）需带入公众号、或遇 LLM 写 base64 爆 token/400 时。不用于生成普通网页/落地页/PPT（用前端或 PPT 类 skill）。
---
...
工作流假设图片是**公网 URL**（`![说明](https://...)` -> 2a `<img src="URL">`，粘贴时公众号直链抓取）。但**图片是本地文件**时（`imgs/01.png`、截图、本地生图），相对路径粘进公众号带不过去。这时走本地配图嵌入，详见 [references/image-embed.md](references/image-embed.md)。

**铁律：绝不让 LLM 在 HTML 里手写 base64。** N 张图转 base64 是数 MB 纯文本，会撑爆 LLM 输出 token、API 返回 400、整段会话崩溃。编码交给浏览器（复制瞬间）或本地脚本。
...
6. **开头引言卡署名**：按文章的**作者或主题**而定——文章有明确作者就写"—— 作者名"；没有明确作者就写本库默认署名"—— 艾启蒙"（同第 7 条）。**不要写"甲木"或其他名字**。
7. **尾部作者签名区（仅末尾一处）**：本库**默认署名写死为「艾启蒙」**，简介固定为「用AI陪伴成长，用技术创造价值！」，不再使用 `{{作者名}}` 占位符。
   - 第一句（作者介绍）：`我是艾启蒙，用AI陪伴成长，用技术创造价值！`——用户在当前请求/偏好里明确给了其他署名或简介时以用户为准；没给就直接写这句默认值，交付时无需再提示替换署名。
   - 第二句（互动引导，通用可保留原样）：`如果你觉得今天这篇有收获，欢迎**点赞、在看、转发**三连，我们下篇见`
   - **原文末尾已有作者签名段**（如"我是 XXX…"）→ 直接沿用原文的署名，不替换成默认值。
...
- **本地图别让 LLM 写 base64**：图片是本地文件要带入公众号时，用 `scripts/build_gzh_html.py` 生成 base64 自包含版（本地脚本跑，无 token 限制）；**绝不让 LLM 在 HTML 里手写 base64**--数 MB 文本会爆输出 token、触发 400、会话崩溃。详见 [references/image-embed.md](references/image-embed.md)。

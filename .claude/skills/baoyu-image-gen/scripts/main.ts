...
Options:
  ...
  --provider google|openai|openrouter|dashscope|zai|minimax|replicate|jimeng|seedream|azure|codex-cli|agnes|modelscope  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
Environment variables:
  ...
  MODELSCOPE_API_KEY        ModelScope API key (falls back to DASHSCOPE_API_KEY)
  ...
  MODELSCOPE_BASE_URL       Custom ModelScope endpoint (default: https://api-inference.modelscope.cn/)
  MODELSCOPE_IMAGE_MODEL    Default ModelScope model (Tongyi-MAI/Z-Image-Turbo)
...
export function getConfiguredProviderRateLimits(
  ...
  for (const provider of ["replicate", "google", "openai", "openrouter", "dashscope", "zai", "minimax", "jimeng", "seedream", "azure", "codex-cli", "agnes", "modelscope"] as Provider[]) {
...
export function detectProvider(args: CliArgs): Provider {
  ...
  const hasModelscope = !!process.env.MODELSCOPE_API_KEY || !!process.env.DASHSCOPE_API_KEY;
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, AZURE_OPENAI_API_KEY+AZURE_OPENAI_BASE_URL, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, ZAI_API_KEY, MINIMAX_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, ARK_API_KEY, AGNES_API_KEY, or MODELSCOPE_API_KEY.\n" +

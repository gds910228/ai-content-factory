---
title: "别再让AI裸写代码了！一套Harness规矩，想全程AI Coding的开发者都能用"
source: "https://mp.weixin.qq.com/s/BF4wEoDLqrAXU-FzBfYXcw"
author:
  - "[[艾启蒙]]"
published:
created: 2026-08-09
description: "AI Coding 不是拼谁的模型强，也不是拼谁的提示词花，而是拼你有没有把 怎么干活 写成规矩。"
tags:
  - "clippings"
---
艾启蒙 AI启蒙学习 *2026年7月13日 11:45*
...
先别急，上周就有朋友找我，说他公司想搞一个“从设计稿到落地”的全程AI Coding实践，领导要求 全程让AI写代码 ，让他总结经验，问我有啥好工具。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/POXYnuKicKs1xDZp9OBiaXqzWUUsT5YsWoozWGSrRiakicdYqQY0Os8KQGz20ics9Xuew7fDS9ULD5w8S8c4I8B0LXdibyaicdT9YlHOk59nZZOydc/640?wx_fmt=png&from=appmsg#imgIndex=0)
...
**捷径** ：装个Harness Creator skill，让Claude Code照着一键生成；

2

**自定义** ：相当于我们以前手撸源码的过程，自己搭。
...
下一步让AI搭Harness需要的其他MD文件（你也可以让它直接读阿里那篇深度文章—— [Harness Engineering：耗时一周，我是如何将应用的AI Coding率提升至90%的](https://mp.weixin.qq.com/s?__biz=MzIzOTU0NTQ0MA==&mid=2247559842&idx=1&sn=71ee08bf0421ad2f1aa4dd7a58901c5f&scene=21&click_id=64#wechat_redirect) ，提取出搭建SOP，再照着搭）。

这背后的逻辑是：既然能让AI干活，那搭Harness这事儿， 也完全可以丢给AI 。

光说不练假把式，拿我 图个简单 小程序 的搭建过程，给你看看这套方法到底怎么跑。

这是个微信小程序，功能不少，但没测试框架、没CI。

我让AI读了本地的Harness落地SOP，它第一件事不是闷头写文件，而是跑去GitHub拉了superpowers仓库当前真实的14个skill清单--不凭记忆， 拿事实说话 。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/POXYnuKicKs38TZI5icJASKqEJXKPMcPupgb3RU8w6qHMTlkJzgPYCuVbe2Wa1HOx8Vaiau09ww333CicrC9wQFAykE4ejdXm95Ng5LdQz1XIFg/640?wx_fmt=png&from=appmsg#imgIndex=1)
...
定好范围，AI就开始落文件：8个skill、4份Rules、1个调度大脑Agent，外加变更管理模板，全落在.harness/ 目录下。

关键是，每个skill都不照搬原文，而是锚定我项目的真实结构去改--我这小程序有20个云函数、14个工具函数、30个页面，AI把这些都摸清了，才动手改造成 项目专属的精简SOP 。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/POXYnuKicKs2JZGWml4WlMWVBE2EZW9hS6ysz2FEiaO2LicG5yvjnCBQXQHlQkgPNH1Us2V6eicNDa6MgiaUbcFyVSj5d4SlO5QIcWusJb6EVnvg/640?wx_fmt=png&from=appmsg#imgIndex=2)
...
### 3个项目实测：搭Harness前后，AI判若两人

我在自己迭代的项目里挑了三个 做Harness搭建 ：

图个简单小程序 富屿扇海居小程序 轻游戏站点
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/POXYnuKicKs3fFZJqyiaaTZZ1uIHia4kvvYle3tcrM54nVz6I23icboHGnO1kEmCr4d6uXzXkepbWUxbaDgAosw5icYtAENcYgGZya412S0xUibu8/640?wx_fmt=png&from=appmsg#imgIndex=3)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/POXYnuKicKs2wz2ZfnLOfiarDlh9nskBlcMS0LRDf4uTIkxSqqrxDFYGKLczIWC2sicS4b0liaicD7bRVTLs76o9AJZzDb6qOHJES4icZldsFHTcc/640?wx_fmt=png&from=appmsg#imgIndex=5)
...
**游戏配置字段类型不统一** --同一个“游戏状态”字段，它这次写成"running"字符串，下次写成数字1，导致后续状态判断逻辑反复出错，查bug查到吐；

2

**游戏资源文件命名随意** --图片、配置文件有的用驼峰、有的用下划线，部署时路径对不上，页面白屏。

前几次我都是手动改，改完就过去了。后来发现这两类问题反复出现，我就在.harness/rules/项目编码规范.md 里补了 两条硬规矩 ：

1

游戏状态字段统一用枚举常量，禁止裸字符串、裸数字；

2

所有游戏资源文件命名统一用下划线，路径放在assets/games/{游戏id}/下。

![图片](https://mmbiz.qpic.cn/mmbiz_png/POXYnuKicKs0EL3ZKqgRt12uicW3a20uDPrS0WgDl8rhuKrKezeVe3zcArnUAPgRRPmyyN19BckGo7wBsYIRqGc193tgBeuJFtL6qX4yl89bE/640?wx_fmt=png&from=appmsg#imgIndex=6)
...
我是 Archer，热衷于分享 AI 独立开发的实战心得。

如果你也在搞 AI Coding，最想拿 Harness 解决哪个“改一个崩一个”的痛点？欢迎 **评论区聊聊** ，咱们一起把规矩补全。

觉得有用，点个 **「在看」** 让更多踩坑的朋友看到。也欢迎 **星标关注** 「AI启蒙学习」，用 AI 陪伴成长，用技术创造价值！
